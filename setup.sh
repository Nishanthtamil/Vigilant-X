...
if ! command -v java &> /dev/null; then
    ...
    if [ "$JAVA_VER" -lt 11 ]; then
        echo "Java version is too old ($JAVA_VER). Joern requires Java 11+ (17 recommended)."
        install_java
    else
...
if ! command -v joern &> /dev/null && [ ! -f "./joern/joern" ]; then
    echo "Joern not found. Downloading local copy (this may take a minute)..."
    mkdir -p joern
...
if command -v docker-compose &> /dev/null || docker compose version &> /dev/null; then
    ...
    echo "Docker Compose not found. You'll need it to run Neo4j."
fi
