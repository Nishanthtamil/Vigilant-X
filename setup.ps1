...
# Speed up downloads by disabling the progress bar
...
if (!(Get-Command joern -ErrorAction SilentlyContinue) -and !(Test-Path "joern\joern.bat")) {
    Write-Host "Joern not found. Downloading local copy (this may take a minute)..." -ForegroundColor Yellow
    if (Test-Path "joern_tmp") { Remove-Item -Recurse -Force "joern_tmp" }
    New-Item -ItemType Directory -Force -Path "joern_tmp" | Out-Null
    
    # Using WebClient for potentially faster download than Invoke-WebRequest
    $url = "https://github.com/joernio/joern/releases/latest/download/joern-cli.zip"
    $output = "joern-cli.zip"
    Invoke-WebRequest -Uri $url -OutFile $output
    
    Write-Host "Extracting Joern..." -ForegroundColor Cyan
    Expand-Archive -Path "joern-cli.zip" -DestinationPath "joern_tmp" -Force
    
    if (Test-Path "joern") { Remove-Item -Recurse -Force "joern" }
